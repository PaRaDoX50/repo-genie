...
from litellm import embedding, token_counter, aembedding

from assistant.base_embed import BaseEmbed
...
class LiteLlmEmbed(BaseEmbed):
    ...
    async def async_create_embedding(self, text):
        if self.delay:
            sleep(self.delay)
        if not text:
            text = "--empty--"
        return await aembedding(**self.lite_llm_embed_completion_options, input=text)
